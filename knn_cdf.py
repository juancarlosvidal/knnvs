...
class Knn(object):
    ...
    def predict_ecdf(self, x, z, k, quantile_value):
        raise NotImplementedError("Abstract method to implement")
...
class KnnVar(Knn):
    ...
    def predict_average(self, x, k=10):
        """
    ...
    def predict_average_ecdf(self, x, z, k=10, quantile_value=0.10):
        assert k > 0, "k must be equal or greater than 1"
        assert 0 < quantile_value < 1, "quantile_value s defined between 0 and 1"
        assert type(x) == np.ndarray, "x must be a numpy array"
        assert type(z) == np.ndarray, "x must be a numpy array"
        yp = self._knna.predict_ecdf(self, x, z, k, quantile_value)
        return yp

    def predict_variance(self, x, k=10):
        """
        The predict method in the KnnBag class is an implementation of the
        predict abstract method from the Knn class. It is specifically
        designed to predict outcomes based on the average of the target values
    ...
    def predict_variance_ecdf(self, x, z, k=10, quantile_value=0.10):
        assert k > 0, "k must be equal or greater than 1"
        assert 0 < quantile_value < 1, "quantile_value s defined between 0 and 1"
        assert type(x) == np.ndarray, "x must be a numpy array"
        assert type(z) == np.ndarray, "x must be a numpy array"
        yp = self._knnv.predict_ecdf(self, x, z, k, quantile_value)
        return yp
...
class KnnBag(Knn):
    ...
    def predict_ecdf(self, x, z, k=10, quantile_value=0.10):
        assert k > 0, "k must be equal or greater than 1"
        assert 0 < quantile_value < 1, "quantile_value s defined between 0 and 1"
        assert type(x) == np.ndarray, "x must be a numpy array"
        assert type(z) == np.ndarray, "x must be a numpy array"
