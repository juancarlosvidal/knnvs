    ...
    for _n in _n_list:
        for _p in _p_list:
            for _i in range(_n_sims):
                ...
                knna1, fa1, ka1, pva1, knnv1, fv1, kv1, pvv1 = initialize_knn(_x, _y, _grida, _gridv,
                                                                              quantile=(1 - _alpha))
                knna2, fa2, ka2, pva2, knnv2, fv2, kv2, pvv2 = no_initialize_knn(_x, _y, _grida, _gridv,
                                                                                 quantile=(1 - _alpha))
                ...
                for i, xi in enumerate(_x):
                    ...
                    plt.plot(z, pecdf1[0], label='ECDF of kNN with feature selection')
                    plt.plot(z, pecdf2[0], label='ECDF of kNN without feature selection')
                    plt.plot(z, norm_cdf, label='Theoretical value')
                    plt.plot(z, norm_cdf_e1, label='CDF of kNN with feature selection')
                    plt.plot(z, norm_cdf_e2, label='CDF of kNN without feature selection')
                    plt.plot(z, r_cdf, label='CDF of GAMLSS')

                    #plt.title('Comparison of the ECDF and CDF of kNN and GAMLSS', fontsize=16)
                    #Comparison between the Empirical Cumulative Distribution Function (ECDF) and the Theoretical Cumulative Distribution Function (CDF) for k-Nearest Neighbors (kNN) and Generalized Additive Models for Location, Scale, and Shape (GAMLSS)
                    plt.xlabel('X', fontsize=10)
                    plt.ylabel('Cumulative probability', fontsize=10)
                    # plt.legend(loc='lower right', bbox_to_anchor=(1, 0), frameon=False)
                    # Removing the legend frame explicitly
