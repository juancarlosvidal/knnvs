...
# def f0(z): return 10
def f0(z):
    return 5 * np.sum(z[0:3])


# def g0(z): return 1.75 * (z[0] > 0.5) + 0.25 * (z[0] <= 0.5)
def g0(z):
    return 5 * np.sum(z[1:4])
...
if __name__ == "__main__":
    ...
    with open("med4_no.csv", "w", newline="") as file:
        ...
        for n, b in n_list:
            for p in p_list:
                for i in range(n_sims):
                    x, y = simulate(n, p)
                    knna, fa, ka, pva, knnv, fv, kv, pvv = no_initialize_knn(
                        x, y, grida, gridv, quantile=(1 - 0.01)
                    )
                    knn = KnnVar(knna, knnv)

                    set1, set2 = set(fa), set(fa_gt)
                    acc_sel_m = len(set1.intersection(set2)) / max(
                        fa.shape[0], fa_gt.shape[0]
                    )
                    set1, set2 = set(fv), set(fv_gt)
                    acc_sel_v = len(set1.intersection(set2)) / max(
                        fv.shape[0], fv_gt.shape[0]
                    )
