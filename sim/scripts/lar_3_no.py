...
from knnvs import KnnBag, KnnVar, no_initialize_knn
import random
import logging
import csv
...
# def f0(z): return 10
def f0(z): return 5 * np.sum(z[0:5])


# def g0(z): return 1.75 * (z[0] > 0.5) + 0.25 * (z[0] <= 0.5)
def g0(z): return 5 * np.sum(z[7:9])
...
#    p_list = [3, 5, 10]
    ...
    str_sim = 'lar3-no-fs-[0:5]-[7:9]'
    fa_gt = np.array([0, 1, 2, 3, 4, 5])   # Ground truth of important variables in mean
    fv_gt = np.array([7, 8, 9])  # Ground truth of important variables in variance

    grida = np.array([5, 10, 20, 50, 100, 200, 500, 1000, 2000])
    gridv = np.array([5, 10, 20, 50, 100, 200, 500, 1000, 2000])

    with open('lar3_no.csv', 'w', newline='') as file:
        ...
        for n, b in n_list:
            for p in p_list:
                for i in range(n_sims):
                    x, y = simulate(n, p)
                    knna, fa, ka, pva, knnv, fv, kv, pvv = no_initialize_knn(x, y, grida, gridv, quantile=(1-0.01))
