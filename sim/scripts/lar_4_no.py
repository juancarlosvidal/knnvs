...
# def f0(z): return 10
def f0(z): return 5 * np.sum(z[0:3])


# def g0(z): return 1.75 * (z[0] > 0.5) + 0.25 * (z[0] <= 0.5)
def g0(z): return 5 * np.sum(z[5:9])
...
#    p_list = [3, 5, 10]
    ...
    str_sim = 'lar4-no-fs-[0:3]-[5:9]'
    fa_gt = np.array([0, 1, 2, 3])   # Ground truth of important variables in mean
    fv_gt = np.array([5, 6, 7, 8, 9])  # Ground truth of important variables in variance

    grida = np.array([5, 10, 20, 50, 100, 200, 500, 1000, 2000])
    gridv = np.array([5, 10, 20, 50, 100, 200, 500, 1000, 2000])

    with open('lar4_no.csv', 'w', newline='') as file:
